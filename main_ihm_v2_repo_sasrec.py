# copied from main.py, only to change the model class
...
from .utils_ihm import data_partition, evaluate, evaluate_valid, WarpSampler


# buck run @mode/opt //sasrec:sasrec_repo -- --dataset=ml-1m --train_dir=ml-1m_repo_sasrec --maxlen=200 --dropout_rate=0.2 --num_epochs 100
...
if not os.path.isdir("experiments/" + args.dataset + "_" + args.train_dir):
    os.makedirs("experiments/" + args.dataset + "_" + args.train_dir)
with open(
    os.path.join("experiments/" + args.dataset + "_" + args.train_dir, "args.txt"), "w"
) as f:
    f.write(
        "\n".join(
            [
...
if __name__ == "__main__":
    ...
    f = open(
        os.path.join("experiments/" + args.dataset + "_" + args.train_dir, "log.txt"),
        "w",
    )
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step in range(
            ...
            _, _, pos_logits, neg_logits = model(u, seq, pos, neg, pos_ih, neg_ih)
        ...
        if epoch == args.num_epochs:
            folder = "experiments/" + args.dataset + "_" + args.train_dir
