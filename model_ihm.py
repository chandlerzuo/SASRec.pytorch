...
class SASRecIHM(torch.nn.Module):
    ...
    def ihm_emb(self, i_seq, ih_seq):
        ...
        mha_output, _ = self.ihm_attn_layer(
            V, V, V, key_padding_mask=ih_mask
        )  # (max_ih_len+1, B*L, C)
        ...
        if self.model == "IHM":
            ihm_gating_weights = self.ihm_gating(
                torch.cat([item_embs, ihm_embs], dim=2)
            )  # (B, L, 1)
        elif self.model == "SASRec":
            ihm_gating_weights = 0
        final_embs = ihm_embs * ihm_gating_weights + item_embs * (
            1 - ihm_gating_weights
        )  # (B, L, C)
        return final_embs
